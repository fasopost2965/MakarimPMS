...
- Un module métier = un dossier sous `backend/src/modules/<domaine>/` avec `<domaine>.module.ts` / `.controller.ts` / `.service.ts` / `dto/`. Modules actuels : `auth`, `rooms` (propriétaire exclusif de `Room`/`RoomType`/`RoomStatusLog`, module feuille sans dépendance, pas de controller propre — voir ci-dessous), `reservations`, `checkin` (= `stay` dans `docs/modules/`, renommage différé), `housekeeping`, `maintenance`, `guests` (contient aussi `CompaniesController`/`CompaniesService` — Company reste une responsabilité du module guests, pas de module séparé), `billing` (Folio/FolioLine/Invoice/TaxRateConfig), `payments` (`Payment`, dépend exclusivement de `billing`), `dashboard`, `audit`.
