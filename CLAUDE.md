...
- Un module métier = un dossier sous `backend/src/modules/<domaine>/` avec `<domaine>.module.ts` / `.controller.ts` / `.service.ts` / `dto/`. Modules actuels : `auth`, `rooms` (propriétaire exclusif de `Room`/`RoomType`/`RoomStatusLog`, module feuille sans dépendance, pas de controller propre — voir ci-dessous), `reservations`, `stay` (renommé depuis `checkin` — `StayService`/`StayController`/`StayModule`, fichiers sous `backend/src/modules/stay/`), `housekeeping`, `maintenance`, `guests` (contient aussi `CompaniesController`/`CompaniesService` — Company reste une responsabilité du module guests, pas de module séparé), `billing` (Folio/FolioLine/Invoice/TaxRateConfig), `payments` (`Payment`, dépend exclusivement de `billing`), `dashboard`, `audit`. **Écart connu (déviation documentée)** : les routes HTTP (`/checkin/*`, `/checkout/:stayId`) et la clé de permission (`checkin:read`/`checkin:write`) du module `stay` sont volontairement restées nommées `checkin` — seuls les identifiants internes (dossier, classes, fichiers) ont été renommés, pour ne toucher ni le frontend (`frontend/src/features/checkin/` reste lui aussi nommé `checkin`, dette de nommage séparée) ni la suite e2e. Ne pas renommer `checkin:read`/`checkin:write` en `stay:*` sans une PR dédiée qui traite RBAC + routes + frontend ensemble.
...
- **Frontières de module** : un module ne doit jamais lire directement les tables d'un autre domaine via Prisma — il passe par le service exposé par le module propriétaire (`exports: […]` + import du module). Exemple : `GuestsService.factures()` appelle `BillingService.findInvoicesByGuestId()` plutôt que d'interroger `prisma.invoice` lui-même. De même, `PaymentsService.createPayment` n'écrit jamais dans `FolioLine` lui-même — il appelle `BillingService.creditFolioLine(folioId, montant, libelle, tx)` **dans la même transaction** que l'écriture du `Payment`, qui crée la ligne `PAIEMENT` créditrice (docs/modules/payments.md §2/§10 : `payments` ne dépend que de `billing`). **Dérogation documentée** : `docs/modules/housekeeping.md` §11 interdit à `housekeeping` de dépendre de `reservations` (protection de la vie privée client), mais le rattrapage quotidien des statuts « relatifs à aujourd'hui » (voir plus bas) a besoin de savoir si une réservation arrive aujourd'hui — faute de cron, ce recalcul ne peut s'exécuter qu'à la lecture. `HousekeepingService` accède donc à `Reservation`/`Stay` exclusivement via les façades en lecture seule `ReservationsService.findConfirmedArrivingToday()` / `StayService.findActiveStayForRoom()` (jamais de Prisma direct) — c'est un écart assumé au référentiel gelé, pas un oubli.
