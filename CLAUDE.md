...
- **Frontières de module** : un module ne doit jamais lire directement les tables d'un autre domaine via Prisma — il passe par le service exposé par le module propriétaire (`exports: […]` + import du module). Exemple : `GuestsService.factures()` appelle `BillingService.findInvoicesByGuestId()` plutôt que d'interroger `prisma.invoice` lui-même. **Dérogation documentée** : `docs/modules/housekeeping.md` §11 interdit à `housekeeping` de dépendre de `reservations` (protection de la vie privée client), mais le rattrapage quotidien des statuts « relatifs à aujourd'hui » (voir plus bas) a besoin de savoir si une réservation arrive aujourd'hui — faute de cron, ce recalcul ne peut s'exécuter qu'à la lecture. `HousekeepingService` accède donc à `Reservation`/`Stay` exclusivement via les façades en lecture seule `ReservationsService.findConfirmedArrivingToday()` / `CheckinService.findActiveStayForRoom()` (jamais de Prisma direct) — c'est un écart assumé au référentiel gelé, pas un oubli.
