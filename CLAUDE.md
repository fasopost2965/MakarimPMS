...
5. **Aucune suppression physique** (`DELETE`) sur les entités métier sensibles — soft delete (`deletedAt`) uniquement, et toute opération sensible laisse une trace d'audit immuable dans `AuditLog` avec motif écrit (≥ 10 caractères). *([ADR-005](docs/ADR-005-Audit-Soft-Delete.md))*
