...
async function main() {
    ...
    for (const { typeNom, dateDebut, dateFin, prixNuit, libelle, } of seasonRatesData) {
    ...
    const taxRates = [
        {
            ...
            mode: client_1.TaxMode.POURCENTAGE,
            taux: 10,
            actif: true,
        ...
        {
            type: 'TAXE_SEJOUR',
            mode: client_1.TaxMode.MONTANT_FIXE,
            taux: 3,
            actif: true,
            collectePourTresor: true,
            applicableParDefaut: true,
    ...
    for (const rate of taxRates) {
        await prisma.taxRateConfig.create({
            data: {
                type: rate.type,
                mode: rate.mode,
                taux: new client_1.Prisma.Decimal(rate.taux),
                actif: rate.actif,
    ...
    for (const policy of cancellationPolicies) {
        await prisma.cancellationPolicy.create({
            data: {
                ...
                pourcentagePenaliteAnnulation: new client_1.Prisma.Decimal(policy.pourcentagePenaliteAnnulation),
                pourcentagePenaliteNoShow: new client_1.Prisma.Decimal(policy.pourcentagePenaliteNoShow),
            },
        });
    }
    ...
    for (const rate of cnssRates) {
        await prisma.cnssRateConfig.create({
            data: {
                ...
                tauxSalarie: new client_1.Prisma.Decimal(rate.tauxSalarie),
                tauxEmployeur: new client_1.Prisma.Decimal(rate.tauxEmployeur),
                ...
                plafondMensuel: rate.plafondMensuel != null
                    ? new client_1.Prisma.Decimal(rate.plafondMensuel)
                    : null,
            },
        });
    }
    ...
    const ALL_ACTIONS = ['read', 'write', 'delete', 'export'];
    const permissions = {};
