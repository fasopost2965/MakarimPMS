...
// Solde dû = somme des charges (HEBERGEMENT/EXTRA/TAXE_SEJOUR/
// AJUSTEMENT_HAUSSE) moins les paiements déjà enregistrés et les baisses
// tarifaires (AJUSTEMENT_BAISSE, DESIGN-009B — traité exactement comme
// PAIEMENT, un ajustement à la baisse réduit le solde dû de la même façon
// qu'un règlement), lignes annulées ignorées. Aucun autre module ne doit
// recalculer ce solde autrement (CLAUDE.md règle 3 : toute charge est une
// ligne de folio, le solde s'obtient toujours en les additionnant).
    ...
    (total, folio) =>
      folio.lignes.reduce((sousTotal, ligne) => {
        ...
        return ligne.type === TypeLigneFolio.PAIEMENT ||
          ligne.type === TypeLigneFolio.AJUSTEMENT_BAISSE
