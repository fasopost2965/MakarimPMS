...
Chantiers : **CH-002** (reset password — terminé), **CH-004** (chiffrement PII — terminé), **CH-011** (route `GET /auth/me` + gating RBAC frontend — terminé), **CH-026** (durcissement secondaire).
...
- **CH-011 — terminé.** Nouvelle route `GET /auth/me` (`AuthController`/`AuthService.me()`) retournant `{ id, email, roleId, roleName, permissions: string[] }` — requête `Permission` fraîche à chaque appel (même garantie que `PermissionsGuard`, jamais mise en cache dans le JWT). Arbitrage de granularité tranché par l'utilisateur en faveur de l'onglet entier, pas de l'action individuelle (RD-009). Côté frontend, `App.tsx` porte l'état `permissions` (pas de `AuthContext` séparé, sur-ingénierie pour un seul consommateur actuel) transmis à `AppSidebar`, qui filtre `NAV_ITEMS` par un nouveau champ `permission`. **Correctif connexe découvert en implémentant** : `lib/api-client.ts` traitait tout `/auth/*` comme public (jamais de tentative de refresh sur 401) — plus vrai pour `GET /auth/me`, corrigé par une liste explicite d'endpoints publics.
...
**Ce qu'il faut tester** : ✅ e2e vérifiant l'absence de `resetToken` dans la réponse HTTP + présence du `NotificationLog` correspondant (CH-002) ; ✅ `GET /auth/me` avec différents rôles (`auth.e2e-spec.ts` — 401 sans token, permissions exactes d'un rôle restreint sans fuite d'un autre module, actions dédiées hors grille pour l'Administrateur, reflet immédiat d'un retrait de permission) + vérification frontend manuelle réelle en navigateur (Chromium/Playwright, 4 rôles, CH-011) ; ✅ test unitaire chiffrement/déchiffrement (`field-encryption.spec.ts`, avec preuve sabotage/restore sur la détection d'altération) + e2e non-régression sur recherche client et export police (CH-004 — l'assertion pré-existante `reporting.e2e-spec.ts` sur le contenu du CSV police a servi de preuve indépendante que le déchiffrement traverse bien les lectures imbriquées).

**Ce qu'il faut documenter** : ✅ `docs/governance/REGISTRE_DECISIONS.md` — RD-004 (CH-002), RD-006 (CH-004), RD-009 (CH-011).
...
Chantiers : **CH-006** (soft-delete centralisé — terminé), **CH-010** (déduplication client), **CH-013** (enums morts), **CH-014** (historique `RoomStatusLog`), **CH-024** (contrainte `RoomNight`), **CH-025** (contraintes `CHECK`).
...
**Ce qu'il faut modifier** :
- **CH-006 — terminé.** Choix technique tranché en faveur de `$extends` (RD-010, `docs/governance/REGISTRE_DECISIONS.md`) — `$use` est déprécié depuis Prisma 5 (projet en Prisma 6.19.3), `$extends` déjà établi par CH-004. Extension chaînée (`.$extends(guestEncryptionExtension(...)).$extends(softDeleteExtension())`), vérifiée en live comme compatible avec l'extension de chiffrement. Recherche exhaustive avant implémentation : aucun code du projet n'écrit jamais `deletedAt` à ce jour — ce mécanisme est un filet de sécurité, sans changement de comportement observable aujourd'hui.
...
**Ce qu'il faut tester** : ✅ suite e2e complète rejouée après CH-006 (123/123 verts, aucune régression malgré le risque le plus élevé de ce domaine) + `backend/test/soft-delete.e2e-spec.ts` dédié (preuve sabotage/restore sur le filtre `findMany`) ; test dédié pour CH-010 (tentative de création d'un doublon rejetée ou signalée selon l'arbitrage) — reste à faire ; test de non-régression du seed (`prisma/seed.ts`) après toute modification de contrainte.

**Ce qu'il faut documenter** : `docs/DATA_DICTIONARY.md` (contraintes ajoutées), `docs/governance/MATRICE_TRACABILITE.md` (CH-006 mis à jour).

**Comment on saura que c'est fini** : `docs/governance/STATUT_MODULES.md` ne référence plus CH-010/CH-013/CH-014 pour `guests`/`rooms` (CH-006 déjà retiré, terminé).

### Domaine RBAC frontend — partie backend (`auth`)
Chantier : **CH-011 — terminé** (route `GET /auth/me` + gating frontend, déjà couvert dans le domaine Sécurité ci-dessus — mentionné ici pour la traçabilité de la dépendance croisée avec le plan frontend, désormais résolue dans les deux sens).
...
5. **CH-011** (`GET /auth/me` + gating RBAC frontend) — ✅ terminé, backend et frontend dans le même chantier (granularité onglet entier, RD-009).
6. **CH-012** (remboursement acompte imputé) — ✅ terminé. Livré dans la foulée de CH-001.
7. **CH-006** — ✅ terminé, sorti de la campagne groupée initialement prévue : contrairement à CH-010/CH-013/CH-014/CH-024/CH-025, ne nécessitait aucune migration de schéma (extension client Prisma pure), donc aucune raison technique de partager leur fenêtre de migration. **CH-010, CH-013, CH-014, CH-024, CH-025** (campagne groupée Intégrité des données restante) — une seule fenêtre de migration.
...
| CH-006 | Un middleware Prisma mal scopé pourrait masquer des lignes que certains écrans légitimes doivent voir (ex. un futur écran d'historique) | *(Résolu)* extension `$extends` (pas de middleware `$use`), portée exhaustivement documentée (12 modèles nommément listés, jamais un filtre implicite) ; suite e2e complète rejouée (123/123, 20 fichiers) sans régression |
...
- **À implémenter** : ~~CH-001~~ (terminé), ~~CH-002~~ (terminé), ~~CH-004~~ (terminé), ~~CH-005~~ (terminé), ~~CH-006~~ (terminé), ~~CH-011~~ (terminé), ~~CH-012~~ (terminé), CH-010, CH-013 (partiel — `ANNULEE_PAR_AVOIR` résolu par CH-001, `StatutSejour.ANNULE` restant), CH-014, CH-024, CH-025 — aucun arbitrage de fond ne remet en cause leur nécessité, seul le « comment » reste à trancher pour certains.
