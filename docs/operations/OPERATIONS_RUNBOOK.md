...
```mermaid
flowchart LR
    DNS["DNS : pms.hotelmakarim.cloud / api.hotelmakarim.cloud -> IP du VPS"] --> NGX["Nginx hôte (hors Docker) : TLS Certbot, routage par server_name"]
    NGX -->|"pms.hotelmakarim.cloud"| FE["Conteneur frontend (nginx:1.27-alpine, port hôte 8081)"]
    NGX -->|"api.hotelmakarim.cloud"| BE["Conteneur backend (NestJS, port hôte 3000)"]
...
*   `FRONTEND_URL` : `https://pms.hotelmakarim.cloud` — seule origine CORS autorisée avec `credentials:true`.
