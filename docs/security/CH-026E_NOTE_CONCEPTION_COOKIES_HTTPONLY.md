...
- **Contrainte de domaine (à documenter dans `README.md`/`operations/OPERATIONS_RUNBOOK.md` lors de l'implémentation)** : `SameSite` définit le « site » par domaine enregistrable (eTLD+1), pas par port ni sous-domaine exact — frontend et backend doivent donc partager le même domaine enregistrable en production (ex. `app.makarim.ma` et `api.makarim.ma`, ou même hôte avec ports différents comme en développement local `localhost:5173`/`localhost:3000`) pour que les cookies partent correctement. Si un déploiement futur héberge le backend sur un domaine totalement distinct, l'authentification cesserait de fonctionner — actuellement non garanti explicitement par la configuration (`FRONTEND_URL`/`VITE_API_URL` sont deux variables indépendantes), à documenter comme prérequis de déploiement plutôt qu'à deviner.
