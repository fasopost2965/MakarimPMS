...
5. **Aucun écran ne doit prétendre à des droits que l'utilisateur n'a pas.** Une fois CH-011 livré, un bouton d'action non autorisé doit être masqué, pas simplement désactivé sans explication ni affiché puis rejeté par un 403 sec — l'échec silencieux ou trompeur est ce que l'audit a identifié comme le problème actuel (Phase 5 §2), la solution ne doit pas remplacer un problème par un autre (bouton visible mais qui échoue).
...
Un nouvel écran n'est prêt pour recette que si, cumulativement : (1) il respecte le pattern chargement/erreur homogène ; (2) ses états vide/erreur/chargement sont explicitement vérifiés, pas seulement le cas nominal ; (3) sa terminologie est cohérente avec le vocabulaire déjà fixé ailleurs dans le produit (backend et autres écrans) ; (4) aucune action qu'il expose n'est accessible à un rôle qui n'en a pas la permission réelle, une fois CH-011 livré.
