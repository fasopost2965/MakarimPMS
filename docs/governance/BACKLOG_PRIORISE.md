...
14. ~~CH-018~~ — ✅ **Terminé** (session courante). Resynchronisation documentation modules — `MODULES_INDEX.md`/`CLAUDE.md` (14/17→21 modules réels) + 8 fiches spec créées (`auth`, `police`, `self-checkin`, `notifications`, `booking-engine`, `document-ocr`, `channel-manager`, `dashboard`) — scope réel élargi validé avec l'utilisateur avant exécution, voir `REGISTRE_CHANTIERS.md` pour le détail.
...
17. ~~CH-022~~ — ✅ **Terminé** (session courante). UI document-ocr — voir `REGISTRE_CHANTIERS.md` pour le détail (corrige au passage un bug bloquant réel : une image corrompue faisait planter tout le process backend, pas seulement la requête).
...
#### Vague 4 — Qualité et fiabilité frontend (Phase 11, `CH-028` à `CH-035`)

Issue d'un audit distinct (`docs/audits/PHASE_11_FRONTEND_QUALITE.md`), mené après la clôture de la quasi-totalité des écrans de la Vague 2/3 — porte sur la qualité structurelle du frontend (tests, accessibilité, sécurité du stockage client, résilience, performance), pas sur des écrans manquants. **Aucun code n'a été écrit pour ces chantiers** : à la demande explicite de l'utilisateur, cette vague reste au stade documentation/gouvernance tant qu'un feu vert d'exécution n'a pas été donné, chantier par chantier ou par lot (même discipline que CH-027). **Exécution désormais organisée en 5 lots A-E, chacun limité à 1-2 sessions** — partitionnement détaillé et plan d'exécution par lot (périmètre, risques, tests, format de compte-rendu) : `docs/frontend-plan/LOTISSEMENT_CHANTIERS_QUALITE.md` et `docs/frontend-plan/PLAN_EXECUTION_LOTS_QUALITE.md`. Ordre recommandé ci-dessous (identique à l'ordre des lots A→E) :

1. **CH-031 — Error boundary transverse**. ✅ **Terminé (Lot A, session courante)**. Le plus rapide (0,5-1 j), aucune dépendance, réduit immédiatement le risque le plus sévère (crash total de l'application sur une erreur de rendu isolée) — voir `REGISTRE_CHANTIERS.md` pour la preuve sabotage/restore en navigateur réel.
2. **CH-028 — Socle de tests automatisés (Vitest + Testing Library)**. ✅ **Terminé pour son socle initial (Lot A, session courante)**. 4 tests ciblés sur un risque réel (RBAC, refresh token/corps vide, upload multipart, affichage financier) — voir `REGISTRE_CHANTIERS.md`. Pratique continue au-delà, pas un chantier clos une fois pour toutes.
3. **CH-026(e) / stockage des tokens `localStorage`** *(chantier déjà existant, enrichi par la Phase 11 — voir `REGISTRE_CHANTIERS.md`, fiche CH-026, pas une nouvelle fiche)*. Sécurité, refonte plus large (cookie `httpOnly` + CSRF + révision du carve-out CORS F4/F6) — à traiter une fois le socle de tests (CH-028) posé, pour pouvoir vérifier la non-régression du flux d'authentification par une suite automatisée plutôt que manuellement.
4. **CH-032 — Composants partagés (dette Lot 0)**. Fondation transverse pour tout le reste — peut être découpé en sous-lots (`table`/`form` en premier, priorité Haute déjà actée dans `COMPOSANTS_PARTAGES_MANQUANTS.md`), chacun appliqué immédiatement à un écran réel existant.
5. **CH-029 — Accessibilité (a11y)**. Se combine naturellement avec CH-032 (composants partagés) — plus simple de rendre accessible un composant générique que huit écrans indépendamment. À mener en parallèle ou juste après CH-032.
6. **CH-030 — Code splitting par onglet**. Indépendant du reste, gain rapide (1 j), peut être inséré à tout moment sans attendre les autres chantiers de cette vague.
7. **CH-034 — Arbitrage responsive/mobile**. Ne nécessite d'abord qu'une décision produit (coût nul), pas du code — à trancher explicitement tôt dans la vague pour ne pas laisser un choix implicite non documenté perdurer, développement conditionnel seulement si l'arbitrage retient l'option « investir ».
8. **CH-033 — Branding et finitions**. Volontairement en dernier — non prioritaire par choix explicite de l'utilisateur (« plus tard »), et dépend d'un travail graphique sur le logo source hors du périmètre de cette session.

*(CH-035 — resynchronisation de `MATRICE_MODULE_API_ECRAN.md` — déjà ✅ terminé, corrigé dans la même session que son versement, ne figure pas dans cet ordre d'exécution.)*
...
Au moment de la création de ce backlog, tous les chantiers étaient au statut **à faire** (session Claude 1, documentation uniquement). **Les 4 chantiers bloquants (CH-001, CH-002, CH-003, CH-004) sont désormais tous terminés** (session de suite, développement effectif) — la Vague 1 du backlog est intégralement close. **La Vague 2 est désormais intégralement close** : CH-005, CH-006, CH-007, CH-008, CH-009, CH-010, CH-011 et CH-012 sont tous terminés (CH-010, le dernier restant, implémenté cette session — index aveugle `pieceIdentiteHash` + détection souple par email/téléphone, RD-011). En Vague 3, CH-013/CH-020/CH-021/CH-023 sont clos (un terminé avec code, trois fermés sans développement — écarts assumés ou statu quo actés, voir `ECARTS_ASSUMES.md`). **La Vague 4 (qualité/fiabilité frontend, Phase 11, CH-028 à CH-035) est en cours d'exécution, lot par lot** (`docs/frontend-plan/LOTISSEMENT_CHANTIERS_QUALITE.md`) — Lot A (CH-031, CH-028) ✅ terminé ; CH-035 (resync documentaire) déjà clos depuis le versement de l'audit ; **Lot B ✅ Terminé** (4 sous-lots B1-B4, 7 composants, session courante — voir `REGISTRE_CHANTIERS.md`, fiche CH-032) ; **Lot C ✅ Terminé** (CH-034 — responsive/mobile, RD-021, tiroir `AppSidebar`/`AppTopbar` ; CH-029 — accessibilité, plugin `jsx-a11y` + focus trap prouvé sur `dialog.tsx` + 3 parcours prioritaires vérifiés au clavier) ; **Lot D en cours** (CH-030 — code splitting par onglet, `React.lazy`+`Suspense`, chunk principal réduit de 571 kB à 237 kB, session courante ✅ terminé ; CH-026(e) — migration des tokens JWT vers un cookie `httpOnly`, le chantier le plus risqué de la vague, reste à faire, nécessite une note de conception CSRF/CORS avant tout code) ; Lot E reste `à faire` (voir `REGISTRE_CHANTIERS.md`, section « Chantiers frontend — issus de l'audit qualité (Phase 11) »). Le suivi de statut se fait dans `REGISTRE_CHANTIERS.md` (champ *Statut* par fiche, section « Suivi d'avancement ») : mettre à jour ce champ, pas ce backlog, quand un chantier avance — ce document liste l'ordre recommandé, pas l'état courant en détail.
