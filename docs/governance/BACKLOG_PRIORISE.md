...
10. **CH-007** — ✅ **Terminé** (session courante). Interface frontend self-checkin (staff) — voir `REGISTRE_CHANTIERS.md` pour le détail (`SelfCheckinPanel.tsx`, corrige au passage un bug latent de `lib/api-client.ts` et la dette technique #6 de `seed.ts`). **CH-008 / CH-009** — Interfaces frontend notifications / channel-manager, restent à faire *(indépendantes entre elles)*
