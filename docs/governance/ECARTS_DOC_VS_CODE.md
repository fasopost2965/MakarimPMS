...
|---|---|---|---|
| `docs/modules/MODULES_INDEX.md`, `CLAUDE.md` | 13-17 modules selon le document | 21 modules réels dans `backend/src/modules/` | Phases 1, 4 §1 |
| `docs/modules/MODULES_INDEX.md` | Module « accounting » listé | Aucune correspondance dans le code backend | Phase 1 *(reconstitué — à reconfirmer)* |
...
| `docs/execution/GO_LIVE_CHECKLIST.md` | Exige une variable `ENCRYPTION_KEY` | N'apparaît dans aucun fichier de code | Phase 5 §3 |
...
| Pas de spec `docs/modules/police.md` dédiée *(à confirmer — absence vérifiée dans le listing `docs/modules/`, non explicitement croisée avec le contenu de chaque fichier pendant l'audit)* | — | Module `police` fonctionnel côté backend depuis F1, sans documentation de module dédiée dans `docs/modules/` | À confirmer (voir CH-018) |
