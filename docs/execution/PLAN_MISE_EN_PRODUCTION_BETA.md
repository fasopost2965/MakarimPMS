...
## 2ter. Migration base de données locale (bac à sable) — résolu (session courante)
Cause réelle confirmée : `_prisma_migrations` contenait un enregistrement fantôme (`20260724070000_ch024_roomnight_exclusivity`, `applied_steps_count: 0`, `finished_at: NULL` — jamais réellement appliqué, probablement une commande interrompue lors d'une session précédente), pas une vraie migration manquante. Consentement utilisateur explicite obtenu (« feu vert ») pour `npx prisma migrate resolve --rolled-back 20260724070000_ch024_roomnight_exclusivity` — exécuté avec succès, `migrate status` confirme "Database schema is up to date!".

**Limite résiduelle découverte en poursuivant (CH-050, pas supposée)** : `prisma migrate dev` continue de refuser toute nouvelle migration sur cette base précise, semble ignorer `rolled_back_at` dans sa logique de détection de dérive côté shadow database — comportement distinct de `migrate status`, qui reste propre. **Contournement fiable identifié et validé deux fois** (CH-050, puis reproductible pour CH-038/039/040/041/048/049) : générer le SQL par diff direct contre la base réelle (pas de shadow database, donc pas d'historique de migrations à réconcilier) puis l'appliquer par le chemin de contrôle standard :
```bash
npx prisma migrate diff --from-url "$DATABASE_URL" --to-schema-datamodel prisma/schema.prisma --script \
  > prisma/migrations/<timestamp>_<nom>/migration.sql
npx prisma migrate deploy
```
Aucune commande destructive, migration versionnée normale dans `prisma/migrations/`, même chemin `migrate deploy` que celui documenté pour la production (`docs/operations/OPERATIONS_RUNBOOK.md` §3). CH-038/039/040/041/048/049 ne sont donc plus bloqués par la base de données — seules les décisions produit encore ouvertes (voir `docs/execution/PLAN_MODULE_FACTURATION.md` pour CH-048/049, `REGISTRE_DECISIONS.md` pour CH-044) les retiennent désormais.
