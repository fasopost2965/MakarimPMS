...
**Statut** : proposition en attente de validation utilisateur — document avant code, conformément à la discipline du projet.
...
| Facture en PDF téléchargeable | **Absent.** `InvoicePrintModal` ne génère aucun fichier serveur — seulement une impression navigateur. Le module `police` a déjà `pdfkit` en dépendance et un générateur PDF réel (`POST /police/:stayId/pdf`, F1) — même bibliothèque réutilisable, mais rien d'équivalent n'existe pour les factures. |
| Envoi facture par email | **Absent** pour les factures spécifiquement, mais l'infrastructure existe : `MailerService.send(to, subject, html)` (dégradation gracieuse si `SMTP_HOST` absent) — **ne supporte pas encore les pièces jointes** (nodemailer le permet nativement, juste jamais câblé). |
...
| Ajout d'une consommation restaurant à la facture chambre | **Backend déjà prêt, frontend manquant.** `POST /folios/:id/lignes` (`billing:write`) existe déjà et fonctionne (`BillingService.addFolioLine`) — mais **aucun bouton nulle part dans le frontend** ne l'appelle (`BillingTabContent.tsx` n'affiche que les lignes existantes, jamais de formulaire d'ajout). |
