...
describe('RoomsToCleanWidget — données réelles uniquement', () => {
  it('sépare les chambres à traiter et celles bloquées pour maintenance', async () => {
    vi.mocked(listRooms).mockResolvedValue([
      ...
      room({ id: 5, numero: '505', statut: 'EN_MAINTENANCE' }),
    ]);
    ...
    expect(screen.getByText('Bloquées pour maintenance')).toBeInTheDocument();
    expect(screen.getByText('Bloquée pour maintenance')).toBeInTheDocument();
